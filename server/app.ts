...
import { getSupabaseStatus } from './db/supabaseClient';
...
export function createApp(): express.Express {
  ...
  // Trust proxy for proper protocol/host resolution behind Vercel, Cloud Run & reverse proxies
  ...
  // CORS and origin headers handling
  ...
  // Helper to extract session ID from cookie, headers, or query parameters
  ...
  const handleOAuthCallback = async (req: Request, res: Response) => {
    ...
    try {
      ...
      // Set cookie configured for cross-origin iframe and production environments
      res.cookie(SESSION_COOKIE_NAME, session.id, {
        httpOnly: true,
        secure: process.env.NODE_ENV === 'production' || req.protocol === 'https',
        sameSite: 'none',
  ...
  // POST /api/ai/categorize - Categorize Single Email and store in Supabase
  ...
  // POST /api/ai/batch-categorize - Batch Categorize Multiple Emails
